	...
	<head>
		...
		so.init(function(){
			...
			function deleteById(ids){
				var index = layer.confirm("确定清除这"+ ids.length +"个用户的属性？",function(){
					var load = layer.load();
						 ...
						 <shiro:hasPermission name="/role/index.shtml">
						  <li class="">
						      <a href="<%=basePath%>/role/index.shtml">
						    	 <i class="glyphicon glyphicon-chevron-right"></i>属性列表
						      </a>
						  </li>
						  </shiro:hasPermission>
						 <shiro:hasPermission name="/role/allocation.shtml">
						  <li class="active dropdown">
						      <a href="<%=basePath%>/role/allocation.shtml" title="属性分配">
						    	 <i class="glyphicon glyphicon-chevron-right"></i>属性分配
						      </a>
						  </li>
						  </shiro:hasPermission>
					...
					<form method="post" action="" id="formId" class="form-inline">
						<div clss="well">
					     ...
					     <span class=""> <%--pull-right --%>
				         	...
				         	<shiro:hasPermission name="/role/clearRoleByUserIds.shtml">
				         		<button type="button" id="deleteAll" class="btn  btn-danger">清空用户属性</button>
			         		</shiro:hasPermission>
				         </span>    
				        </div>
					<hr>
							...
							<c:when test="${page != null && fn:length(page.list) gt 0}">
								<c:forEach items="${page.list}" var="it">
									<tr>
										...
										<td>
											<shiro:hasPermission name="/role/addRole2User.shtml">
												<i class="glyphicon glyphicon-share-alt"></i><a href="javascript:selectRoleById(${it.id});">选择属性</a>
											</shiro:hasPermission>
										</td>
									</tr>
								</c:forEach>
			  ...
			  <div class="modal-dialog modal-sm" role="document">
			    <div class="modal-content">
			      <div class="modal-header">
			        ...
			        <h4 class="modal-title" id="selectRoleLabel">添加属性</h4>
			      </div>
