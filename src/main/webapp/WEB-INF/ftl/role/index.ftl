	...
	<head>
		...
		<script >
			...
			function deleteById(ids){
				var index = layer.confirm("确定这"+ ids.length +"个属性？",function(){
					var load = layer.load();
			...
			function addRole(){
				var name = $('#name').val(),
					type = $('#type').val();
				if($.trim(name) == ''){
					return layer.msg('属性名称不能为空。',so.default),!1;
				}
				if(!/^[a-z0-9A-Z]{6}$/.test(type)){
					return layer.msg('属性类型为6数字字母。',so.default),!1;
				}
				<#--loding-->
				var load = layer.load();
			...
			<div class="row">
				...
				<div class="col-md-10">
						...
						<div clss="well">
					      <div class="form-group">
					        <input type="text" class="form-control" style="width: 300px;" value="${findContent?default('')}" 
					        			name="findContent" id="findContent" placeholder="输入属性类型 / 属性名称">
					      </div>
					     <span class=""> <#--pull-right -->
				         	<button type="submit" class="btn btn-primary">查询</button>
				         	<@shiro.hasPermission name="/role/addRole.shtml">
				         		<a class="btn btn-success" onclick="$('#addrole').modal();">增加属性</a>
				         	</@shiro.hasPermission>
				         	<@shiro.hasPermission name="/role/deleteRoleById.shtml">
					...
					<table class="table table-bordered">
						...
						<#else>
							<tr>
								<td class="text-center danger" colspan="4">没有找到属性</td>
							</tr>
						</#if>
					</table>
					<#if page?exists>
				  ...
				  <div class="modal-dialog" role="document">
				    <div class="modal-content">
				      <div class="modal-header">
				        ...
				        <h4 class="modal-title" id="addroleLabel">添加属性</h4>
				      </div>
				      <div class="modal-body">
				        <form id="boxRoleForm">
				          <div class="form-group">
				            <label for="recipient-name" class="control-label">属性名称:</label>
				            <input type="text" class="form-control" name="name" id="name" placeholder="请输入属性名称"/>
				          </div>
				          <div class="form-group">
				            <label for="recipient-name" class="control-label">属性类型:</label>
				            <input type="text" class="form-control" id="type" name="type"  placeholder="请输入属性类型  [字母 + 数字] 6位">
				          </div>
				        </form>
				      </div>
