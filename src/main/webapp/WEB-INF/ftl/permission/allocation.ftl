		...
		<script >
			so.init(function(){
				...
				so.id('deleteAll').on('click',function(){
					...
					if(!checkeds.length){
						return layer.msg('请选择要清除的属性。',so.default),!0;
					}
					var array = [];
			...
			function deleteById(ids){
				var index = layer.confirm("确定清除这"+ ids.length +"个属性的权限？",function(){
					var load = layer.load();
						...
						<div clss="well">
					      <div class="form-group">
					        <input type="text" class="form-control" style="width: 300px;" value="${findContent?default('')}" 
					        			name="findContent" id="findContent" placeholder="输入属性名称 / 属性类型">
					      </div>
					     <span class=""> <#--pull-right -->
				         	<button type="submit" class="btn btn-primary">查询</button>
				         	<@shiro.hasPermission name="/permission/clearPermissionByRoleIds.shtml">
				         		<button type="button" id="deleteAll" class="btn  btn-danger">清空属性权限</button>
				         	</@shiro.hasPermission>
				         </span>    
				        </div>
					<hr>
